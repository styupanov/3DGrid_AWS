...
const CesiumMap = () => {
  ...
  const loadTileset = async (level) => {
    ...
    try {
      ...
      // )
      tileset.maximumScreenSpaceError = 6 // Увеличьте для снижения детализации и повышения производительности
      tileset.maximumMemoryUsage = 16384 // Максимальное использование памяти в МБ
      tileset.cullWithChildrenBounds = true // Использовать объединенные границы дочерних тайлов для отсечения

      tileset.dynamicScreenSpaceError = false // Включить динамическую ошибку экранного пространства
      ...
      tileset.baseScreenSpaceError = 2048 // Базовая ошибка экранного пространства
      tileset.skipScreenSpaceErrorFactor = 64 // Фактор ошибки для пропуска уровней
      tileset.skipLevels = 3 // Количество уровней для пропуска
      ...
      tileset.foveatedTimeDelay = 1.0;
      tileset.foveatedMinimumScreenSpaceErrorRelaxation = 1.0;
      // tileset.foveatedScreenSpaceError = false // Приоритизация загрузки тайлов в центре экрана
      ...
      tileset.cacheBytes = 1024 * 1024 * 2048; // 512 MB
      tileset.maximumCacheOverflowBytes = 1024 * 1024 * 2048; // 128 MB оверфлоу
  ...
  useEffect(() => {
    ...
    if (routeInfo.startCell) {
      handleSearch(routeInfo.startCell, false, Color.GREEN)
    }

    if (routeInfo.finishCell) {
      handleSearch(routeInfo.finishCell, false, Color.RED)
    }

  }, [routeInfo.startCell, routeInfo.finishCell])
